...
#
# Keep the positive topic query separate from the exclusions so the same
# exclusions also apply when a user adds a non-duplicate keyword query.
...
(
  (
    ...
    (
      (
        "Estradiol"[Mesh]
        OR "Progesterone"[Mesh]
        OR estradiol[Title/Abstract]
        OR oestradiol[Title/Abstract]
        OR progesterone[Title/Abstract]
      )
      AND
      (
...
(
  (
    ...
    (
      ...
      OR adenocarcinoma*[Title/Abstract]
    )
  )
  OR EOC[Title/Abstract]
  OR HGSOC[Title/Abstract]
  OR LGSOC[Title/Abstract]
)
""".strip()


# Retained as a combined compatibility constant for duplicate-detection and
# any external imports. The actual default PubMed expression below keeps the
# two exclusions as separate ``AND NOT`` clauses, exactly like PUBMED_TERM.
...
# Literal exclusion entries that are already global/default exclusions. Keep
# this list separate from generic words that occur only inside a compound rule.
# For example, ``cancer*`` appears under ``ovary OR ovarian`` in the default
# query, so a user entry ``not cancer`` is still a new global exclusion.
...
DEFAULT_QUERY_LABEL = (
    "Ovary/ovulation + inflammation/immunity in humans, excluding ovarian cancer"
)
...
def build_augmented_pubmed_query(
    ...
    """Build the exact default PubMed query plus optional user additions.

    With no user input, this returns the same Boolean expression as
    ``PUBMED_TERM``: the curated PMID branch OR the filtered topic branch.
    Positive user terms broaden the topic branch with ``OR``. User exclusions
    are added as another ``AND NOT`` filter without changing the built-in
    animal-only and ovarian-cancer filters.
    """

    if not user_keywords and not user_exclusions:
        return DEFAULT_PUBMED_QUERY

    topic_query = f"({DEFAULT_PUBMED_TOPIC_QUERY})"
    if user_keywords:
        user_query = " OR ".join(
            f"({keyword_to_pubmed_clause(item)})" for item in user_keywords
        )
        topic_query = f"({topic_query}) OR ({user_query})"
    ...
    if user_exclusions:
        ...
        user_exclusion_clause = f" AND NOT ({user_exclusion_query})"
...
def _request_esearch(
    ...
    *,
    ...
    # Always use POST for ESearch. The built-in Boolean query is roughly 1,400
    # characters before URL encoding but well over common safe URL lengths
